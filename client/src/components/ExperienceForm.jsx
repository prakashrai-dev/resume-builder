import { Briefcase, Loader, Loader2, Plus, Sparkles, Trash2 } from 'lucide-react' // FIX: Added 'Sparkles' to the import list as it was used but not imported.
                    ...
                    {data.map((experience, index)=>(
                        <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                            ...
                            <div className='grid md:grid-cols-2 gap-3'>
                                ...
                                <input value={experience.position || ""} onChange={(e)=>updateExperience(index,"position",e.target.value)} type="text" placeholder='Job Title' className='px-3 py-2 text-sm rounded-lg' /> {/* FIX: Corrected typo from 'positon' to 'position'. */}
    ...
    <Loader2 className='w-3 h-3 animate-spin' />
                    ...
                    Enhance with AI
                                ...
                                <textarea value={experience.description || ""} onChange={(e) => updateExperience(index, "description", e.target.value)}
                                    rows={4} className="w-full text-sm px-3 py-2 rounded-lg resize-none" placeholder="Describe your key responsibilities and achievements..." />
                            </div>
                        </div>
                    ))}
                </div>
            )}
        </div> // FIX: Added the missing closing </div> tag for the main container.
    )
}
